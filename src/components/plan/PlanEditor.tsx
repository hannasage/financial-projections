...
import { Modal, Button, ButtonGroup, Toggle } from '@hannasage/projection-ui';
...
function SectionHead({ label, onAdd, addLabel = '+ Custom' }: SectionHeadProps) {
  const COLORS = useColors();
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
      <span style={{ fontSize: 10, letterSpacing: 2, color: COLORS.muted, textTransform: 'uppercase' }}>{label}</span>
      {onAdd && <Button variant="secondary" size="sm" onClick={onAdd}>{addLabel}</Button>}
    </div>
  );
}
...
export function PlanEditor({ initialScenario, color, markers, onSave, onCancel, isSaving, footerPosition = 'fixed' }: Props) {
  ...
  }, [debts, purchases, raises, investments, recurringCharges, addLibraryDebt, addLibraryPurchase, addLibraryRaise, addLibraryInvestment, addLibraryRecurringCharge]);
  ...
  const resolvedInvestments     = useMemo(() => [...library.investments.filter(i => !excludedInvestmentIds.includes(i.id)), ...investments], [library.investments, excludedInvestmentIds, investments]);
  const resolvedRecurringCharges = useMemo(() => [...library.recurringCharges.filter(c => !excludedRecurringChargeIds.includes(c.id)), ...recurringCharges], [library.recurringCharges, excludedRecurringChargeIds, recurringCharges]);
  ...
  const scenario: Scenario = useMemo(() => ({
    startMonthIdx: safeStartMonthIdx, startYear: safeStartYear,
    envelope: init.envelope, startSavings: init.startSavings, startAge: init.startAge,
    horizonYears: init.horizonYears, returnMode: init.returnMode, hysaRate: init.hysaRate,
    taxPct: init.taxPct, baseSalary: init.baseSalary, housingCost: init.housingCost,
    monthlyAllowance: init.monthlyAllowance ?? 0,
    retirementAge: init.retirementAge, retirementEnvelope: init.retirementEnvelope,
  ...
  const chartInvest = useMemo(() => data.map(row => {
    ...
    for (const inv of resolvedInvestments) { pt[`inv_${inv.id}`] = row.investmentBalancesById[inv.id] ?? 0; }
    return pt;
  ...
  const nowInvContrib = useMemo(() => resolvedInvestments.reduce((s, i) => {
    const st = absMo(i.startYear ?? mergedScenario.startYear, i.startMonthIdx ?? mergedScenario.startMonthIdx, mergedScenario.startYear, mergedScenario.startMonthIdx);
    if (st > 0) return s;
  ...
  const scenarioOverviewChips = useMemo(() => {
    ...
    const nd = resolvedDebts.length;             if (nd) chips.push({ key: 'debts', label: `${nd} active debt${nd === 1 ? '' : 's'}` });
    const nr = resolvedRecurringCharges.length;  if (nr) chips.push({ key: 'rec',   label: `${nr} recurring` });
    const np = resolvedPurchases.length;         if (np) chips.push({ key: 'pur',   label: `${np} major purchase${np === 1 ? '' : 's'}` });
    const ni = resolvedInvestments.length;       if (ni) chips.push({ key: 'inv',   label: `${ni} investment bucket${ni === 1 ? '' : 's'}` });
    const nz = resolvedRaises.length;            if (nz) chips.push({ key: 'raise', label: `${nz} raise scenario${nz === 1 ? '' : 's'}` });
    return chips;
  ...
  // Year table clip options
  const tableClipOptions: ButtonGroupOption[] = [
    ...[3, 5, 7, 10].filter(y => y < mergedScenario.horizonYears).map(y => ({ value: String(y), label: `${y}yr` })),
    { value: 'all', label: 'All' },
  ];

  const forkBtn = (onClick: () => void) => (
    <Button variant="secondary" size="sm" onClick={onClick} title="Copy to scenario as a custom item">⎘ fork</Button>
  );

  const handleSave = () => onSave(scenario);

  return (
    <div style={{ '--plan-accent': accent, background: COLORS.bg, minHeight: '100vh', color: COLORS.text, fontFamily: 'var(--ui-font)' } as React.CSSProperties}>
      <div style={{ maxWidth: 780, margin: '0 auto', paddingBottom: 80 }}>
        ...
        <section className="sec" aria-label="Global settings summary">
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 14 }}>
            ...
            <a href="/io" style={{
              ...
              padding: '8px 14px', borderRadius: 6, fontSize: 10, fontWeight: 700,
              letterSpacing: 1, textTransform: 'uppercase' as const, textDecoration: 'none',
              color: COLORS.textOnAccent, background: accent,
              border: `1px solid ${accent}`, fontFamily: 'var(--ui-font)',
            }}>Edit in I/O</a>
          </div>

          <div style={{
            borderRadius: 10, border: `1px solid ${COLORS.border}`, borderLeft: `4px solid ${accent}`,
            background: COLORS.surface, overflow: 'hidden', boxShadow: `0 12px 40px ${COLORS.bg}88`,
          }}>
            <div style={{ padding: '10px 12px 12px', borderBottom: `1px solid ${COLORS.border}`, background: COLORS.faint }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(112px, 1fr))', gap: 6 }}>
                {[
                  ...
                  <div key={k} style={{ padding: '7px 9px', borderRadius: 6, background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
            ...
            <div style={{ padding: '14px 16px 16px' }}>
              ...
              <p style={{ fontSize: 10, color: COLORS.muted, margin: '0 0 12px', lineHeight: 1.65 }}>
                Starting cash <strong style={{ color: COLORS.text }}>{money(mergedScenario.startSavings)}</strong>
              ...
              <div style={{ fontSize: 11, color: COLORS.text, fontVariantNumeric: 'tabular-nums' }}>
                ...
                {envelopeDeductions.map(r => (
                  <div key={r.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '6px 0', borderBottom: `1px solid ${COLORS.border}` }}>
                ...
                <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${COLORS.border}` }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {scenarioOverviewChips.map(c => (
                      <span key={c.key} style={{ fontSize: 10, padding: '5px 11px', borderRadius: 999, border: `1px solid ${COLORS.border}`, background: COLORS.faint, color: COLORS.text }}>
                        {c.label}
                      </span>
                    ))}
                  </div>
        ...
        {/* Library item helper */}
        {([
          { kind: 'debt'      as CustomKind, label: '💳 Active Debts',       items: library.debts,             excluded: excludedDebtIds,           toggle: toggleDebt,           fork: forkDebt,           color: COLORS.accent, formatSub: (d: { label: string; payment: number; payoffMonthIdx: number; payoffYear: number }) => `−${money(d.payment)}/mo · off ${MONTHS[d.payoffMonthIdx]} ${d.payoffYear}` },
          { kind: 'recurring' as CustomKind, label: '📎 Recurring bills',     items: library.recurringCharges, excluded: excludedRecurringChargeIds, toggle: toggleRecurringCharge, fork: forkRecurringCharge, color: COLORS.muted,  formatSub: (c: { amount: number }) => `−${money(c.amount)}/mo` },
          { kind: 'purchase'  as CustomKind, label: '🛒 Major Purchases',     items: library.purchases,        excluded: excludedPurchaseIds,        toggle: togglePurchase,       fork: forkPurchase,       color: COLORS.orange, formatSub: (p: { payment: number; type: string; year: number }) => `−${money(p.payment)}/mo · ${p.type === 'house' ? '🏠' : '🚗'} ${p.year}` },
          { kind: 'investment' as CustomKind, label: '📊 Investments',        items: library.investments,      excluded: excludedInvestmentIds,      toggle: toggleInvestment,     fork: forkInvestment,     color: accent,        formatSub: (i: { initialAmount: number; annualReturnPct: number; monthlyContribution: number }) => `${money(i.initialAmount)} start · ${i.annualReturnPct}% · +${money(i.monthlyContribution)}/mo` },
          { kind: 'raise'     as CustomKind, label: '📈 Raise Scenarios',     items: library.raises,           excluded: excludedRaiseIds,           toggle: toggleRaise,          fork: forkRaise,          color: COLORS.blue,   formatSub: (r: { monthIdx: number; year: number; salary: number }) => `${MONTHS[r.monthIdx]} ${r.year} · ${money(r.salary)}/yr` },
        ] as const).map(({ kind, label, items, excluded, toggle, fork, color, formatSub }) => {
          const addFns: Record<CustomKind, () => void> = {
            debt: addDebt, recurring: addRecurringCharge, purchase: addPurchase, investment: addInvestment, raise: addRaise,
          };
          const customItems: Record<CustomKind, { id: string }[]> = {
            debt: debts, recurring: recurringCharges, purchase: purchases, investment: investments, raise: raises,
          };
          const changeFns: Record<CustomKind, (id: string, patch: never) => void> = {
            debt: changeDebt as never, recurring: changeRecurringCharge as never, purchase: changePurchase as never, investment: changeInvestment as never, raise: changeRaise as never,
          };
          const rmFns: Record<CustomKind, (id: string) => void> = {
            debt: rmDebt, recurring: rmRecurringCharge, purchase: rmPurchase, investment: rmInvestment, raise: rmRaise,
          };

          return (
            <section key={kind} className="sec" aria-label={label}>
              <SectionHead label={label} onAdd={addFns[kind]} />
              {(items as { id: string; label?: string }[]).length > 0 && (
                <div style={{ marginBottom: 8 }}>
                  <span style={{ ...S.label, display: 'block', marginBottom: 6 }}>From I/O</span>
                  {(items as { id: string; label?: string }[]).map(item => {
                    const off = excluded.includes(item.id);
                    return (
                      <div key={item.id} role="button" tabIndex={0}
                        ...
                        style={{
                          ...
                          border: `1px solid ${off ? COLORS.border : color}40`,
                          background: off ? 'transparent' : `${color}08`,
                          marginTop: 6, opacity: off ? 0.5 : 1, transition: 'all 0.12s',
                        }}>
                        <span style={{ color: off ? COLORS.muted : color, fontSize: 13 }}>{off ? '○' : '●'}</span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 12, fontWeight: 500, color: off ? COLORS.muted : COLORS.text }}>{(item as { label?: string }).label || 'Unnamed'}</div>
                          <div style={{ fontSize: 10, color: COLORS.muted }}>{formatSub(item as never)}</div>
                        </div>
                        {forkBtn(() => fork(item as never))}
                      </div>
                    );
                  })}
                </div>
              )}
              {(items as unknown[]).length === 0 && (customItems[kind] as unknown[]).length === 0 && (
                <p style={{ fontSize: 11, color: COLORS.muted, marginTop: 4, fontStyle: 'italic' }}>
                  Add in <a href="/io" style={{ color: COLORS.accent, textDecoration: 'none' }}>I/O</a> or use + Custom for scenario-only items.
                </p>
              )}
              {/* Cascade toggle for debts */}
              {kind === 'debt' && resolvedDebts.length >= 2 && (
                <div style={{ margin: '8px 0' }}>
                  <Toggle
                    checked={cascadeDebts}
                    onChange={setCascadeDebts}
                    label={cascadeDebts ? '⛓ cascade on' : '⛓ cascade freed payments'}
                  />
                  <p style={{ fontSize: 10, color: COLORS.muted, marginTop: 8, lineHeight: 1.55, maxWidth: 560 }}>
                    Cascade rolls freed payments from paid-off debts into the next remaining debt in list order.
                  </p>
                </div>
              )}
              {(customItems[kind] as { id: string }[]).map(item => (
                <div key={item.id} data-custom-item={item.id}>
                  <CustomItemWrapper
                    kindLabel={`Custom ${KIND_LABEL[kind]} · scenario only`}
                    onSaveToLibrary={() => saveCustomToLibrary(kind, item.id, (item as { label?: string }).label ?? '')}
                    savedToLibrary={Boolean(savedToLibrary[item.id])}
                  >
                    {kind === 'debt' && <DebtItem d={item as Debt} startYear={mergedScenario.startYear} onChange={p => changeFns.debt(item.id, p as never)} onRemove={() => rmFns.debt(item.id)} />}
                    {kind === 'recurring' && <RecurringChargeItem c={item as RecurringCharge} startYear={mergedScenario.startYear} horizonYears={mergedScenario.horizonYears} onChange={p => changeFns.recurring(item.id, p as never)} onRemove={() => rmFns.recurring(item.id)} />}
                    {kind === 'purchase' && <PurchaseItem p={item as Purchase} startYear={mergedScenario.startYear} startMonthIdx={mergedScenario.startMonthIdx} housingCost={mergedScenario.housingCost} horizonYears={mergedScenario.horizonYears} onChange={p => changeFns.purchase(item.id, p as never)} onRemove={() => rmFns.purchase(item.id)} />}
                    {kind === 'investment' && <InvestmentItem i={item as Investment} planStartYear={mergedScenario.startYear} planStartMonthIdx={mergedScenario.startMonthIdx} horizonYears={mergedScenario.horizonYears} onChange={p => changeFns.investment(item.id, p as never)} onRemove={() => rmFns.investment(item.id)} />}
                    {kind === 'raise' && <RaiseItem r={item as Raise} startYear={mergedScenario.startYear} taxPct={mergedScenario.taxPct} baseSalary={mergedScenario.baseSalary} onChange={p => changeFns.raise(item.id, p as never)} onRemove={() => rmFns.raise(item.id)} />}
                  </CustomItemWrapper>
                </div>
              ))}
            </section>
        ...
        <section className="sec" aria-label="Cash savings milestones" style={{ overflowX: 'hidden', maxWidth: '100%' }}>
          ...
          <p style={{ fontSize: 10, color: COLORS.dim, marginBottom: 8, lineHeight: 1.55 }}>
            Charts reflect simplified timing and taxes—directionally useful, not exact balances.
          </p>
          ...
          <div className="mg" style={{ maxWidth: '100%' }}>
            {milestones.map(s => (
              <div key={s.m} style={{
                padding: '12px 10px', minWidth: 0, maxWidth: '100%', boxSizing: 'border-box',
                ...
                <div className="syne" title={money(snap(s.m)?.liquidTotal ?? 0)}
                  style={{ fontSize: 'clamp(15px, 4.2vw, 22px)', fontWeight: 800, color: s.hi ? accent : COLORS.text, lineHeight: 1.1, marginBottom: 5, wordBreak: 'break-word', overflowWrap: 'anywhere' }}>
        ...
        <section className="sec" aria-label="Cash savings trajectory chart">
          ...
          <p style={{ fontSize: 11, color: COLORS.muted, marginBottom: 14 }}>Liquid cash only. 🛒 purchase · ✓ loan paid</p>
          ...
          <ResponsiveContainer width="100%" height={230}>
            <AreaChart data={chart} margin={{ top: 8, right: 6, left: 0, bottom: 0 }}>
              ...
              <XAxis dataKey="decimalYr" type="number" domain={[chartMinYr, chartStart + chartHorizon]}
                tickCount={Math.min(chartHorizon + 1, 16)}
                tickFormatter={v => (v % 1 < 0.05) ? String(Math.round(v)) : ''}
                tick={{ fill: COLORS.muted, fontSize: 11, fontFamily: 'var(--ui-font)' }} axisLine={false} tickLine={false} />
              <YAxis tickFormatter={shortK} tick={{ fill: COLORS.muted, fontSize: 11, fontFamily: 'var(--ui-font)' }} axisLine={false} tickLine={false} width={44} />
              <Tooltip content={<ChartTooltip markers={markers} />} />
              {renderMarkerOverlay({ markers: markers ?? [], minDecimalYr: chartMinYr, maxDecimalYr: chartStart + chartHorizon, colors: COLORS, keyPrefix: 'mk-liq' })}
              {purchaseMarkers.map((m, i) => <ReferenceLine key={`buy-${i}`} x={m.buyDecimalYr} stroke={COLORS.orange} strokeDasharray="3 3" strokeWidth={1} label={{ value: '🛒', fill: COLORS.orange, fontSize: 10, position: 'top' }} />)}
              {purchaseMarkers.filter(m => m.withinHorizon).map((m, i) => <ReferenceLine key={`paid-${i}`} x={m.paidDecimalYr} stroke={COLORS.blue} strokeDasharray="3 3" strokeWidth={1} label={{ value: '✓', fill: COLORS.blue, fontSize: 10, position: 'top' }} />)}
              <Area type="monotone" dataKey="liquidTotal" stroke={accent} strokeWidth={2} fill="url(#chartGrad)" dot={false} activeDot={{ r: 4, fill: accent, strokeWidth: 0 }} />
            </AreaChart>
          </ResponsiveContainer>
        </section>
        ...
        <section className="sec" aria-label="Debt paydown chart">
          ...
          <ResponsiveContainer width="100%" height={190}>
            <AreaChart data={chart} margin={{ top: 8, right: 6, left: 0, bottom: 0 }}>
              ...
              {renderMarkerOverlay({ markers: markers ?? [], minDecimalYr: chartMinYr, maxDecimalYr: chartStart + chartHorizon, colors: COLORS, keyPrefix: 'mk-debt' })}
              <Area type="monotone" dataKey="debtOutstanding" stroke={COLORS.red} strokeWidth={2} fill="url(#debtGrad)" dot={false} activeDot={{ r: 4, fill: COLORS.red, strokeWidth: 0 }} />
            </AreaChart>
          </ResponsiveContainer>
        </section>
        ...
        <section className="sec" aria-label="Investment balances chart">
          ...
          <p style={{ fontSize: 11, color: COLORS.muted, marginBottom: 14 }}>Stacked balances: compounding and monthly adds.</p>
                ...
                <AreaChart data={chartInvest} margin={{ top: 8, right: 6, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip
                    content={({ active, payload: tipPayload }) => {
                      ...
                      return (
                        <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '10px 14px', fontFamily: 'var(--ui-font)', fontSize: 10, minWidth: 180 }}>
                          <div style={{ color: accent, fontWeight: 600, marginBottom: 6 }}>Age {row.ageFloor} · {MONTHS[mo]} {row.yr}</div>
                          ...
                          {activeMarkers.length > 0 && (
                            <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 6, marginTop: 6 }}>
                              <div style={{ color: COLORS.muted, fontSize: 9, letterSpacing: 1, marginBottom: 4 }}>ACTIVE PHASE{activeMarkers.length === 1 ? '' : 'S'}</div>
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                                {activeMarkers.map(m => {
                                  const c = resolveMarkerColor(m.color, COLORS);
                                  const endStr = m.endYear != null && m.endMonthIdx != null ? `${MONTHS[m.endMonthIdx]} ${m.endYear}` : '→';
                                  return (
                                    <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 6, lineHeight: 1.3 }}>
                                      ...
                                      <span style={{ color: COLORS.muted, fontSize: 9 }}>{MONTHS[m.startMonthIdx]} {m.startYear} → {endStr}</span>
                                    </div>
                                  );
                                })}
                              </div>
                  ...
                  {renderMarkerOverlay({ markers: markers ?? [], minDecimalYr: chartMinYr, maxDecimalYr: chartStart + chartHorizon, colors: COLORS, keyPrefix: 'mk-inv' })}
                  {resolvedInvestments.map((inv, idx) => {
                    const c = planPalette[idx % planPalette.length].value;
                    return <Area key={inv.id} type="monotone" stackId="inv" dataKey={`inv_${inv.id}`} name={inv.label || `Account ${idx + 1}`} stroke={c} strokeWidth={1.5} fill={`url(#invGrad-${inv.id})`} dot={false} />;
                  })}
                </AreaChart>
              </ResponsiveContainer>
            </>
        ...
        <section className="sec" aria-label="Net worth chart">
          ...
          <p style={{ fontSize: 11, color: COLORS.muted, marginBottom: 14 }}>Savings plus optional purchase market values minus all debts.</p>
          {markers && markers.length > 0 && <MarkerLegend markers={markers} compact />}
          <ResponsiveContainer width="100%" height={190}>
            <AreaChart data={chart} margin={{ top: 8, right: 6, left: 0, bottom: 0 }}>
              <defs>
              ...
              {renderMarkerOverlay({ markers: markers ?? [], minDecimalYr: chartMinYr, maxDecimalYr: chartStart + chartHorizon, colors: COLORS, keyPrefix: 'mk-nw' })}
              {purchaseMarkers.map((m, i) => <ReferenceLine key={`nw-buy-${i}`} x={m.buyDecimalYr} stroke={COLORS.orange} strokeDasharray="3 3" strokeWidth={1} label={{ value: '🛒', fill: COLORS.orange, fontSize: 10, position: 'top' }} />)}
              {purchaseMarkers.filter(m => m.withinHorizon).map((m, i) => <ReferenceLine key={`nw-paid-${i}`} x={m.paidDecimalYr} stroke={COLORS.blue} strokeDasharray="3 3" strokeWidth={1} label={{ value: '✓', fill: COLORS.blue, fontSize: 10, position: 'top' }} />)}
              <Area type="monotone" dataKey="netWorth" stroke={COLORS.blue} strokeWidth={2} fill="url(#nwGrad)" dot={false} activeDot={{ r: 4, fill: COLORS.blue, strokeWidth: 0 }} />
            </AreaChart>
          </ResponsiveContainer>
        </section>
        ...
        <section className="sec" aria-label="Year-by-year breakdown">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, gap: 8, flexWrap: 'wrap' }}>
            ...
            {tableClipOptions.length > 1 && (
              <ButtonGroup
                options={tableClipOptions}
                value={tableYears === null ? 'all' : String(tableYears)}
                onChange={v => setTableYears(v === 'all' ? null : Number(v))}
                size="sm"
              />
            )}
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, minWidth: 520 }}>
              <thead>
                <tr style={{ borderBottom: `1px solid ${COLORS.border}` }}>
                  {[
                    ['Age',   'Your age'], ['Year', 'Calendar year'], ['Savings', 'Cash / HYSA balance'],
                    ['Invest','Investment account balances'], ['To cash','Net change in cash this month'],
                    ['Debt −','Monthly debt payments'], ['Loans −','Monthly loan payments'], ['Active','Active purchase loans'],
                  ].map(([h, desc]) => (
                    <th key={h} scope="col" title={desc} style={{ padding: '6px 10px 8px', textAlign: 'left', color: COLORS.muted, fontWeight: 500, fontSize: 10, letterSpacing: 1, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {yearly.filter(d => tableYears == null || d.m <= tableYears * 12).map((d, i) => (
                  <tr key={d.m} style={{ borderBottom: `1px solid ${COLORS.border}18`, background: i % 2 === 0 ? `${COLORS.surface}80` : 'transparent' }}>
                    ...
                    <td style={{ padding: '8px 10px', color: d.debtBurden > 0 ? COLORS.red : COLORS.muted }}>{d.debtBurden > 0 ? `−${money(d.debtBurden)}` : '—'}</td>
                    <td style={{ padding: '8px 10px', color: d.purchaseOutflow > 0 ? COLORS.orange : COLORS.muted }}>{d.purchaseOutflow > 0 ? `−${money(d.purchaseOutflow)}` : '—'}</td>
                    <td style={{ padding: '8px 10px', fontSize: 10, color: COLORS.muted }}>{d.activePurchases.length > 0 ? d.activePurchases.join(', ') : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      >
        {copiedInfo && (
          <>
            <strong style={{ color: 'var(--ui-text)' }}>{copiedInfo.itemName}</strong> was added to your global I/O library as a new {KIND_LABEL[copiedInfo.kind]}.
